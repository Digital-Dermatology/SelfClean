...
class TestSelfCleanCleaner(unittest.TestCase):
    ...
    # Approximate-path API tests live in `test_approximate_path_near_duplicates.py`
    # (schema, planted-duplicate top pair, exact-vs-approx parity at K=N-1).
    # The previous `approx_near_duplicates` DataFrame return shape has been
    # collapsed into the standard `near_duplicates` (scores, indices) tuple.
