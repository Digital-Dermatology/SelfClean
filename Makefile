...
.PHONY: build
build:  ##@Utils builds the package
	@python3.9 -m build
...
test: _build  ##@Test run all tests in the project
	$(DOCKER_CMD) /bin/bash -c "python3 -m coverage run -m pytest tests --junitxml=report.xml; coverage report -i --include=selfclean/* --omit="selfclean/core/*"; coverage xml -i --include=selfclean/* --omit="selfclean/core/*";"

.PHONY: unittest
unittest: _build  ##@Test run all unittests in the project
	$(DOCKER_CMD) /bin/bash -c "python3 -m coverage run -m pytest tests --junitxml=report.xml --ignore=tests/integration_tests; coverage report -i --include=selfclean/* --omit="selfclean/core/*"; coverage xml -i --include=selfclean/* --omit="selfclean/core/*";"
